...
def model(x, y, classes):
    ...
    perf_dict = cross_validate(
        ...
        cv=cv,
        scoring={
            "{}".format(classes[n]):f1_class_n(n, np.arange(len(classes))) for n in range(len(classes))},
        n_jobs=cv,
    )
    return perf_dict
...
def make_performance_table(x_set, y_set, dataset_names):
    ...
    for i in range(3):
        ...
        classes.append("All")
        train_means, train_stds, test_means, test_stds, classes = compute_oos_performance(perf_dict, classes)
        for j in range(4):
