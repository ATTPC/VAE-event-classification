...
class LatentModel:
    ...
    def __init__(self, X, latent_dim, beta, mode_config):
        ...
        if len(self.data_shape) == 3 or len(self.data_shape) == 4:
            ...
            raise ValueError(
                """Wrong input dimensions expected
                    x_train to have DIM == 3 or DIM == 4 got DIM == {}""".format(
                    len(self.data_shape)
                )
            )
    ...
    def linear(
        self, x, output_dim, regularizer=tf.contrib.layers.l2_regularizer, lmbd=0.1
    ):
        """
        ...
        """
        w = tf.get_variable(
            "w", [x.get_shape()[1], output_dim], regularizer=regularizer(lmbd)
        )
    ...
    def clustering_layer(self, inputs):
        ...
        q = 1.0 / (1.0 + (tf.reduce_sum(tmp, axis=2) / self.alpha))
    ...
    def pretrain(self, sess, epochs, minibatch_size):
        """
        ...
        earlystop = tf.keras.callbacks.EarlyStopping(
            monitor="val_loss", patience=4, min_delta=1e-7
        )
        ...
        for i in range(epochs):
            ...
            if i % 10 == 0:
                ...
                """
            to_earlystop = self.earlystopping(smooth_loss, all_lx, None, i)
            if to_earlystop:
                return
    ...
    def train(
        ...
        writer = tf.summary.FileWriter(
            "../loss_records/tensorboard/run_{}".format(run), sess.graph
        )
        ...
        for i in range(epochs):
            ...
            if self.train_classifier:
                ...
                if verbose:
                    print(
                        "Epoch {} | Lx = {:5.2f} | Lz = {:5.2f} | clf cost {:5.2f} | \
    ...
    def update_clusters(self, i, sess, data_dir, model_dir):
        """
    ...
    def evaluate_cluster(self, sess, i=None):
        ...
        fmt = ".2f"
        thresh = centroids.max() / 2.0
        for i in range(centroids.shape[0]):
            for j in range(centroids.shape[1]):
                ax.text(
                    j,
                    i,
                    format(centroids[i, j], fmt),
                    ha="center",
                    va="center",
    ...
    def run_clf_batch(self, sess, batch_ind):
        ...
        clf_cost, clf_acc, _, _, _ = sess.run(self.clf_fetches, clf_feed_dict)
        return clf_cost, clf_acc

    def evaluate_classifier(self, sess, all_clf_loss, logloss_train, i):
    ...
    def compile_model(self, graph_kwds=None, loss_kwds=None):
        """
        Paramters 
        ---------
    ...
    def compute_gradients(self, optimizer_class, opt_args=[], opt_kwds={}):
        """
        Parameters:
        ----------
        ...
        self.fetches = []
        self.fetches.extend(
            [self.Lx, self.Lz, self.z_seq, self.dec_state_seq, self.train_op]
        )
    ...
    def storeResult(self, sess, feed_dict, data_dir, model_dir, i):
        """
        Paramters 
        ---------
        ...
        filename = (
            data_dir + "/simulated/canvasses" + epoch + ".npy"
            if self.simulated_mode
            else data_dir + "/canvasses" + epoch + ".npy"
        )
        np.save(filename, canvasses)

        model_fn = (
            model_dir + "/draw_attn" + epoch + ".ckpt"
            if self.use_attention
            else model_dir + "/draw_no_attn" + epoch + ".ckpt"
        )
        if self.simulated_mode:
            model_fn = (
                model_dir + "/simulated/draw_attn" + epoch + ".ckpt"
                if self.use_attention
                else model_dir + "/simulated/draw_no_attn" + epoch + ".ckpt"
            )

        self.saver.save(sess, model_fn)

        ref_fn = (
            data_dir + "/simulated/references" + epoch + ".npy"
            if self.simulated_mode
            else data_dir + "/references" + epoch + ".npy"
        )
    ...
    def generate_latent(self, sess, save_dir, X_tup, save=True):
        ...
        if save:
            for i, X in enumerate(X_tup):
                ...
                dec_fn = (
                    "train_decoder_states.npy" if i == 0 else "test_decoder_states.npy"
                )
    ...
    def generate_samples(
        self, save_dir, n_samples=100, rerun_latent=False, load_dir=None
    ):
        ...
        if rerun_latent:
            ...
            latent_fn = (
                load_dir + "/simulated/latent/train_latent.npy"
                if self.simulated_mode
                else load_dir + "/latent/train_latent.npy"
            )
            dec_state_fn = (
                load_dir + "/simulated/train_decoder_states.npy"
                if self.simulated_mode
                else load_dir + "/train_decoder_states.npy"
            )
        ...
        for t in range(self.T):
            ...
            if not rerun_latent:
                mu, sigma = (0, 1) if t < 1 else (0.0, 1)
                sample = np.random.normal(
                    mu, sigma, (n_samples, self.latent_dim)
                ).astype(np.float32)
            else:
                sample = (
                    latent_samples[t, batch_size : 2 * batch_size, :]
            ...
            if rerun_latent:
                dec_state = (
                    decoder_states[t, :, self.batch_size : 2 * self.batch_size, :]
                    .reshape((2, self.batch_size, dec_size))
                    .astype(np.float32)
                )
