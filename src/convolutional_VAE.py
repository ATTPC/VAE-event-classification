    ...
    def _ModelGraph(
        ...
        if self.use_vgg:
            ...
            for i in range(self.dense_layers):
                h1 = Dense(units, kernel_regularizer=k_reg)(h1)
                units /= 2
        ...
        de1 = tf.keras.layers.Reshape(
            (deconv_shape, deconv_shape, self.filter_arcitecture[-1])
        )(de1)
    ...
    def _ModelLoss(self, reconst_loss=None, scale_kl=False):
        ...
        if self.pretrain_simulated:
            ...
            self.clf_acc = tf.metrics.accuracy(
                tf.math.argmax(self.y_batch, axis=-1), tf.math.argmax(self.q, axis=-1)
            )
